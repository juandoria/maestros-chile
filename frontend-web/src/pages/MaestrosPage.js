    ...
    return (
      <div style={s.centrado}>
        <div style={s.errorCaja}>
          ...
          <button onClick={() => window.location.reload()} style={s.botonVerde}>
            Intentar de nuevo
          </button>
        </div>
      </div>
    ...
    <div style={{ ...s.pagina, padding: isMobile ? '24px 16px 60px' : '36px 24px 80px' }}>
      ...
      {maestros.length === 0 ? (
        <div style={s.vacioCaja}>
          ...
          <button onClick={() => navigate('/')} style={s.botonVerde}>
            Hacer otra búsqueda
          </button>
        </div>
      ) : (
...
const s = {
  ...
  nombre: { fontSize: 19, fontWeight: '700', color: '#0f2a22', margin: '0 0 3px' },
  oficio: { fontSize: 15, color: '#4b7062', margin: '0 0 3px' },
  precio: { fontSize: 17, fontWeight: '700', color: '#1D9E75', margin: 0 },
